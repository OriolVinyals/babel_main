...
class SNRReader:
    ...
    def DumpAudioDiagnostics(self,dir_name='./data/',top_k=10,bot_k=10):
        ...
        for utt in utt_largest:
            ...
            out_file = './data/' + repr(i) + 'large_snr_' + os.path.basename(file_id).split('.')[0] + '.wav'
        ...
        for utt in utt_smallest:
            ...
            t_end = float(utt.split('_')[-1])/self.samp_period
            file_id = self.list_files[self.map_utt_idx[utt_id]]
            out_file = './data/' + repr(i) + 'small_snr_' + os.path.basename(file_id).split('.')[0] + '.wav'
            util.cmdconvert(file_id, out_file, t_beg, t_end)
            i+=1
        #glob level diag
        ...
        for utt_id in glob_largest:
            file_id = self.list_files[self.map_utt_idx[utt_id]]
            out_file = './data/glob_large_snr_' + os.path.basename(file_id).split('.')[0] + '.wav'
        ...
        for utt_id in glob_smallest:
            file_id = self.list_files[self.map_utt_idx[utt_id]]
            out_file = './data/glob_small_snr_' + os.path.basename(file_id).split('.')[0] + '.wav'
            util.cmdconvert(file_id, out_file)
...
if __name__ == '__main__':
    list_files = './data/audio.dev_bengali_flp.list'
